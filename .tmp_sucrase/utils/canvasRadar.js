 ...
 */
 function drawRadar(ctx, dimensions, centerX, centerY, radius, showLabels = true) {
  const levels = 5;
  ...
  // 绘制数据区域
  const points = KEYS.map((key, index) => getPoint(index, _nullishCoalesce(_optionalChain([dimensions, 'optionalAccess', _ => _[key]]), () => ( 5))));
  ctx.beginPath();
 ...
 * @param {number} radius
 */
 function drawRadarWithLegend(ctx, dimensions, centerX, centerY, radius) {
  ...
  KEYS.forEach((key, index) => {
    const value = _nullishCoalesce(_optionalChain([dimensions, 'optionalAccess', _2 => _2[key]]), () => ( 0));
    const col = index % 2;
...
} exports.drawRadarWithLegend = drawRadarWithLegend;

exports.LABELS = LABELS; exports.KEYS = KEYS; exports.NAVY = NAVY;