 ...
 */
 async function uploadShareReport({ results, history, userId }) {
  try {
    // 生成分享码
    ...
    // 插入数据库
    const { data, error } = await _supabase.supabase.from('share_reports').insert(reportData).select('share_code').single();
...
} exports.uploadShareReport = uploadShareReport;
 ...
 */
 async function getShareReport(shareCode) {
  try {
