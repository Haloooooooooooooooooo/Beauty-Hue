 ...
 */
 async function uploadShareCard(blob) {
  try {
    ...
    const { data, error } = await _supabase.supabase.storage
      .from(_supabase.STORAGE_BUCKET)
    ...
    // 生成带过期时间的签名 URL（7天有效）
    const { data: urlData, error: urlError } = await _supabase.supabase.storage
      .from(_supabase.STORAGE_BUCKET)
...
} exports.uploadShareCard = uploadShareCard;