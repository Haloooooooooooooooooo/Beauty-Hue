...
function normalizeBaseUrl(url) {
 ...
 function isUsingLocalShareOrigin() {
  const configuredUrl = normalizeBaseUrl(import.meta.env.VITE_PUBLIC_APP_URL);
  if (configuredUrl) {
    return false;
  }
...
} exports.isUsingLocalShareOrigin = isUsingLocalShareOrigin;
...
function decompressData(data) {
  ...
  const decompressedResults = results.map((r) => ({
    ...
    dimensions: r.d
      ...
      : null,
    seasonName: _optionalChain([_seasonColors.SEASONS, 'access', _ => _[r.k], 'optionalAccess', _2 => _2.nameCN]) || '',
  }));
 ...
 * @returns {string} 分享链接
 */
 function generateGuestShareUrl(results, history) {
  const data = { results, history };
  const encoded = encodeShareData(data);
  const baseUrl = getShareBaseUrl();
  return `${baseUrl}/share?d=${encoded}`;
} exports.generateGuestShareUrl = generateGuestShareUrl;
 ...
 * @returns {string} 分享链接
 */
 function generateUserShareUrl(shareId) {
  const baseUrl = getShareBaseUrl();
  return `${baseUrl}/r/${shareId}`;
} exports.generateUserShareUrl = generateUserShareUrl;
