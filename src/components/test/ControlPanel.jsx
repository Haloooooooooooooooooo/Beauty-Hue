import { Camera, CheckCircle, Circle, RefreshCcw, XCircle } from 'lucide-react';
...
export default function ControlPanel({
  ...
  return (
    <div className="flex flex-col gap-4 items-center h-full justify-center w-full max-w-[200px]">
      ...
      <div className="w-[40px] h-[1px] bg-black/10 my-2" />
      ...
      <div className="flex flex-col gap-3 w-full">
        ...
        <button
          ...
          className={`glass-btn flex items-center justify-center gap-2 transition-all
            ${userScore === 0 ? 'bg-white/60 border-navy/20 shadow-inner scale-95' : ''}`}
        >
      ...
      <div className="w-[40px] h-[1px] bg-black/10 my-4" />

      <button onClick={onScreenshot} className="glass-btn flex items-center justify-center gap-2 w-full">
        <Camera className="w-5 h-5" />
        <span>截图</span>
      </button>

      <button onClick={onReselectPhoto} className="glass-btn flex items-center justify-center gap-2 w-full">
