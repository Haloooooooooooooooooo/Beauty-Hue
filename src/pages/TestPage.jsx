  ...
  const handleScore = async (val) => {
    ...
    setTimeout(() => {
      if (round < sequence.length - 1) {
        ...
        const nextScores = { ...scores, [currentColor.seasonKey]: (scores[currentColor.seasonKey] || 0) + userScore };
        ...
        <AnimatePresence>
          {showToast && (
            ...
            >
              ...
              <span className="text-navy font-semibold tracking-wide">📸 截图已成功保存到本地</span>
            </motion.div>
          )}
        </AnimatePresence>
