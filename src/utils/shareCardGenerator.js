...
import { drawRadar } from './canvasRadar';
import { SEASONS } from '../data/seasonColors';
import { LABELS, KEYS, NAVY } from './canvasRadar';
...
export async function generateShareCard(results, userImage, systemHistory) {
  ...
  if (primarySeason && primaryResult) {
    ...
    // 雷达图
    drawRadar(ctx, primaryResult.dimensions, radarCenterX, radarCenterY, radarRadius, true);
    ...
    KEYS.forEach((key, index) => {
      const value = primaryResult.dimensions?.[key] || 0;
  ...
  // ========== 次季型 ==========
  if (secondaryResult) {
    const secondarySeason = SEASONS[secondaryResult.key];
    ...
    drawRadar(ctx, secondaryResult.dimensions, radarCenterX, radarCenterY, radarRadius, true);

    const infoX = CONFIG.padding + 180;
    ctx.font = 'bold 18px Inter, system-ui, sans-serif';
    ctx.fillStyle = NAVY;
    ctx.textAlign = 'left';
    ctx.fillText(secondarySeason?.nameCN || '', infoX, cardContentY + 10);

    ctx.font = '12px Inter, system-ui, sans-serif';
    ctx.fillStyle = CONFIG.muted;
    ctx.fillText(secondarySeason?.name || '', infoX, cardContentY + 28);
